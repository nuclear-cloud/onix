...
class BaseDataAdapter(ABC):
    ...
    @staticmethod
    def safe_str(value: Any, default: str = None, max_length: int = None) -> Optional[str]:
    ...
    @staticmethod
    def safe_int(value: Any, default: int = None, min_val: int = None, max_val: int = None) -> Optional[int]:
    ...
    @staticmethod
    def safe_decimal(value: Any, default: Decimal = None) -> Optional[Decimal]:
...
class YakabooDataAdapter(BaseDataAdapter):
    ...
    def validate(self, raw_data: Dict[str, Any]) -> ValidationResult:
        ...
        if not isbn:
            result.add_error('isbn13', 'No valid ISBN-13 found in barcode, book_isbn, or book_isbn_label')
    ...
    def transform(self, raw_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            product = ProductDTO(
                ...
                # Title
                title=self.safe_str(raw_data.get('name'), default='Без назви', max_length=500),
                subtitle=self.safe_str(raw_data.get('short_description'), max_length=500),
                
                # Product form
                product_form_code=self._map_product_form(raw_data),
                
                # Physical
                page_count=self.safe_int(raw_data.get('book_page_count'), min_val=1, max_val=50000),
                ...
                metadata={
                    'yakaboo_id': raw_data.get('id'),
                    'yakaboo_sku': raw_data.get('sku'),
    ...
    def _extract_contributors(self, raw_data: Dict, ctx: MappingContext) -> List[ContributorDTO]:
        ...
        for i, author in enumerate(author_labels):
            ...
            try:
                if isinstance(author, dict):
                    ...
                    if name:
                        contributors.append(ContributorDTO(
    ...
    def _extract_subjects(self, raw_data: Dict, ctx: MappingContext) -> List[SubjectDTO]:
    ...
    def _extract_text_content(self, raw_data: Dict, ctx: MappingContext) -> List[TextContentDTO]:
    ...
    def _extract_media_files(self, raw_data: Dict, ctx: MappingContext) -> List[MediaFileDTO]:
